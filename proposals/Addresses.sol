...
contract Addresses is IAddresses, Test {
    ...
    function printRecordedAddresses() external view {
        ...
        for (uint256 i = 0; i < recordedAddresses.length; i++) {
            console.log(
                "Recorded",
                _addresses[recordedAddresses[i].name][recordedAddresses[i].chainId].addr,
                recordedAddresses[i].name
            );
        }
    }
    ...
    function _getAddress(string memory name, uint256 chainId) private view returns (address addr) {
        ...
        require(
            addr != address(0),
            string(abi.encodePacked("Address: ", name, " not set on chain: ", chainId.toString()))
        );
    }
