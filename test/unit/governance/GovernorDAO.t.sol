...
contract UnitTestGovernorDAO is Test {
    ...
    GovernorDAO private governor;

    uint256 private constant _TIMELOCK_MIN_DELAY = 12345;
    uint256 private constant _VOTING_DELAY = 0;
    uint256 private constant _VOTING_PERIOD = 100_000;
    uint256 private constant _PROPOSAL_THRESHOLD = 50_000_000e18;
    uint256 private constant _QUORUM = 200_000_000e18;
    ...
    function setUp() public {
        ...
        core.createRole(Roles.DAO_GOVERNOR_PROTOCOL_ROLE, Roles.ADMIN);
        core.createRole(Roles.GUARDIAN, Roles.ADMIN);
        core.grantRole(Roles.DAO_GOVERNOR_PROTOCOL_ROLE, addresses.tokenGovernorAddress);
        core.grantRole(Roles.GUARDIAN, addresses.guardianAddress);
        vm.stopPrank();

        /// Prank as this update the timelock
        vm.startPrank(address(this));
        timelock.grantRole(timelock.TIMELOCK_ADMIN_ROLE(), address(governor));
        timelock.grantRole(timelock.EXECUTOR_ROLE(), address(governor));
        timelock.grantRole(timelock.PROPOSER_ROLE(), address(governor));
        timelock.grantRole(timelock.CANCELLER_ROLE(), addresses.guardianAddress);
    ...
    function testSuccessfulProposal() public {
        ...
        payloads[0] = abi.encodeWithSelector(UnitTestGovernorDAO.__dummyCall.selector, 12345);

        /// propose a new vote
        token.mockSetVotes(address(this), _QUORUM);
        uint256 proposalId = governor.propose(targets, values, payloads, "Vote for 12345");
        assertEq(uint256(governor.state(proposalId)), uint256(IGovernor.ProposalState.Pending));
        assertEq(uint256(governor.proposalSnapshot(proposalId)), block.number + _VOTING_DELAY);
        assertEq(uint256(governor.proposalDeadline(proposalId)), block.number + _VOTING_PERIOD);
        ...
        assertEq(timelock.isOperationDone(timelockId), false);
        governor.execute(targets, values, payloads, keccak256("Vote for 12345"));
    ...
    function testSetVotingDelayNonTokenGovernor() public {
    ...
    function testSetVotingPeriodNonTokenGovernor() public {
        vm.prank(addresses.userAddress);
        vm.expectRevert("CoreRef: no role on core");
        governor.setVotingPeriod(12);
        assertEq(governor.votingPeriod(), 100000);
    }
    ...
    function testSetProposalThresholdNonTokenGovernor() public {
        vm.prank(addresses.userAddress);
        vm.expectRevert("CoreRef: no role on core");
        governor.setProposalThreshold(34);
        assertEq(governor.proposalThreshold(), 5_0000_000e18);
    }
    ...
    function testSetQuorumNonTokenGovernor() public {
        ...
        assertEq(governor.quorum(999), 200_000_000e18);
    }
    ...
    function testRelay() public {
        ...
        uint256[] memory values = new uint256[](1);
        bytes[] memory payloads = new bytes[](1);
        payloads[0] = abi.encodeWithSelector(Governor.relay.selector, address(this), 10 ether, bytes(""));

        /// propose a new vote
        token.mockSetVotes(address(this), _QUORUM);
        uint256 proposalId = governor.propose(targets, values, payloads, "relay ETH back");
        vm.roll(block.number + _VOTING_DELAY + 1);
        vm.warp(block.timestamp + 10);
        governor.castVote(proposalId, uint8(GovernorCountingSimple.VoteType.For));
        vm.roll(block.number + _VOTING_PERIOD + 1);
        ...
        governor.execute(targets, values, payloads, keccak256("relay ETH back"));
