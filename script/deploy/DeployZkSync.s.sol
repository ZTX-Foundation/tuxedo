...
import {RateLimiter} from "@protocol/nfts/RateLimiter.sol";
...
contract DeployZkSync is Script {
    function run() external {
        ...
        // nftContractAddresses[0] = address(0x16280Bb89C1d8812E9AA724D7E6cCA988be321C0);
        // nftContractAddresses[1] = address(0x181EE014eA2fa80256F8E96a0f5Ef6147586F010);
        // nftContractAddresses[2] = address(0xEfEa0c8dEf5dB7442538e2C51e2dB92B44C62D3a);

        // First, set up NFT contract addresses to whitelist
        address[] memory nftContractAddresses = new address[](3);
        nftContractAddresses[0] = address(0x16280Bb89C1d8812E9AA724D7E6cCA988be321C0); // Consumables
        nftContractAddresses[1] = address(0x181EE014eA2fa80256F8E96a0f5Ef6147586F010); // Placeables
        nftContractAddresses[2] = address(0xEfEa0c8dEf5dB7442538e2C51e2dB92B44C62D3a); // Wearables

        // Libraries are linked automatically during compilation
        // No need to explicitly deploy them with 'new'
        console2.log("Deploying implementation with linked libraries...");

        // Step 1: Deploy the implementation contract
        address globalReentrancyLockAddress = address(0xc152d69DA77af4044e4F50c6bdEe367d0B2F0c42);
        ERC1155AutoGraphMinterImpl implementation = new ERC1155AutoGraphMinterImpl(
            address(_core),
            globalReentrancyLockAddress
        );
        console2.log("ERC1155AutoGraphMinterImpl deployed at:", address(implementation));

        // Step 2: Deploy the proxy (tiny contract for ZKSync)
        console2.log("Deploying proxy...");
        address adminMultisig = address(0x5Ec41e3a9c712D0BBC26d2CbA0E653c5d2cc982C);
        ERC1155AutoGraphMinterProxy proxy = new ERC1155AutoGraphMinterProxy(
            address(implementation),
            adminMultisig
        );
        console2.log("ERC1155AutoGraphMinterProxy deployed at:", address(proxy));

        // Step 3: Initialize the implementation through the proxy
        console2.log("Initializing proxy...");
        ERC1155AutoGraphMinterImpl(address(proxy)).initialize(
            ...
            address(0x0C8AD0e8349fd7662Db2EAC0cF4F1c7AB8558155), // Payment recipient
